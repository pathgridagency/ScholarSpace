      ...
      <div className="border-b border-gray-100 px-8 py-3">
        <div className="flex gap-2">
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${!filter ? "bg-indigo-50 text-indigo-700" : "text-gray-500 hover:bg-gray-50"}`}
          >
            All
          </button>
          {Object.entries(typeLabels).map(([key, label]) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${filter === key ? "bg-indigo-50 text-indigo-700" : "text-gray-500 hover:bg-gray-50"}`}
            >
              {label}
            </button>
          ))}
            ...
            {listings.map((listing) => (
              <div key={listing.id} className="card-static p-5">
                <div className="flex items-start justify-between">
                  <span className={`rounded-full border px-2.5 py-0.5 text-xs font-medium ${typeColors[listing.type]}`}>
                    {typeLabels[listing.type]}
                  </span>
                  <span className="text-sm font-semibold text-gray-900">${Number(listing.price).toFixed(2)}</span>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-gray-50 pt-3">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${listing.status === "AVAILABLE" ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
                    {listing.status}
                  </span>
                </div>
              </div>
